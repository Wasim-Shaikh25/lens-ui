...
//   // Iterate over contactDetail array to check each industryType
//   for (const detail of formData.contactDetail) {
...
//     if (formData.contactDetail && formData.contactDetail.length > 0) {
//       formData.contactDetail[formData.contactDetail.length - 1].lastUpdatedOn = dateTime;
//       formData.contactDetail[formData.contactDetail.length - 1].insertedOn = dateTime;
//     }
...
export const handleSubmit = async (e, formData, navigate, setErrors, validateField) => {
  ...
  // Validate each entry in contactDetail array
  formData.contactDetail.forEach((detail, index) => {
    if (!detail.contactPerson || detail.contactPerson.trim() === "") {
      validationErrors[`contactDetail[${index}].contactPerson`] = "Contact Person cannot be empty.";
    }
    if (!detail.industryType || detail.industryType.trim() === "") {
      validationErrors[`contactDetail[${index}].industryType`] = "Industry Type cannot be empty.";
    }
    if (!detail.mobileNumber || detail.mobileNumber.trim() === "") {
      validationErrors[`contactDetail[${index}].mobileNumber`] = "Mobile Number cannot be empty.";
    }
    if(!/^\d{0,10}$/.test(detail.mobileNumber)){
      validationErrors[`contactDetail[${index}].mobileNumber`] = "Mobile Number length cannot be more than 10.";
    }

    if (!detail.designation || detail.designation.trim() === "") {
      validationErrors[`contactDetail[${index}].designation`] = "designation cannot be empty.";
    }
    if (!detail.customerAddress || detail.customerAddress.trim() === "") {
      validationErrors[`contactDetail[${index}].customerAddress`] = "Customer bAddress cannot be empty.";
    }
  });
  ...
  console.log("Error length is ",Object.keys(validationErrors));
  ...
  if (Object.keys(validationErrors).length === 0) {
    ...
    try {
      const res = await axiosInstance.post(`/lens/customer/save`, formData);
      console.log("response is", res.data);
      navigate(`/Customer/${encodeURIComponent(res.data[0].referenceValue)}`);
    } catch (err) {
      console.log("Error during form submission:", err);
    }
  }
